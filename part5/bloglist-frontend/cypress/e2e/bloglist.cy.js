  ...
  describe('When logged in', function() {
    ...
    describe('Several blogs exist', function() {
      beforeEach(function() {
        ...
        cy.contains('Go To Statement Considered Harmful').parent().find('#view').click()
        cy.contains('Canonical string reduction').parent().find('#view').click()
      })

      it('Users can like a blog', function() {
        cy.get('.blogContent').contains('React patterns').parent().find('#like-button').click()
        cy.get('.blogContent').contains('React patterns').should('contain', '1')
      })

      it('Blog can be deleted by the creator', function() { 
        cy.get('.blogContent').contains('React patterns').parent().contains('delete').click()
      ...
      it('Blog with the most likes is displayed first', function() {
        ...
        cy.get('.blogContent').contains('React patterns').should('contain', '2')

        cy.likeBlog('Go To Statement Considered Harmful')
        cy.likeBlog('Go To Statement Considered Harmful')
        cy.likeBlog('Go To Statement Considered Harmful')
        cy.get('.blog').eq(0).should('contain', 'React patterns')
        cy.get('.blogContent').contains('Go To Statement Considered Harmful').should('contain', '3')
